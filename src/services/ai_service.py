...
class AIService:
    ...
    async def summarize_and_label(self, article: SEArticle) -> SELabeledSummary:
        ...
        if settings.ai_dry_run:
            logger.info("AI_DRY_RUN=true: skipping real LLM call for %r", article.title)
            return self._dry_run_summary(article)
    ...
    @staticmethod
    def _dry_run_summary(article: SEArticle) -> SELabeledSummary:
        """Deterministic, offline stand-in for a real LLM call.

        Used when AI_DRY_RUN=true so the rest of the pipeline (fetch,
        dedup, storage, digest rendering, the web UI) can be exercised
        end-to-end for free, with no API key and no rate limit. This is
        never used for the real submitted digest -- development only.
        Deliberately not cached: a dry-run result must never contaminate
        the real content_hash cache that a genuine LLM call would populate.
        """
        preview = " ".join(article.content.split())[:180]
        return SELabeledSummary(
            summary=f"[DRY RUN -- no LLM called] {preview}...",
            topic=TopicEnum.OTHER,
