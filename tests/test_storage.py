...
def make_transaction_cm() -> AsyncMock:
    """A fake `conn.transaction()` async context manager."""
    cm = AsyncMock()
...
def make_acquire_cm(conn: AsyncMock) -> AsyncMock:
    """A fake `pool.acquire()` async context manager yielding `conn`."""
    cm = AsyncMock()
...
def mock_connection() -> AsyncMock:
    """A fake asyncpg Connection, wired up with a working `.transaction()`."""
    conn = AsyncMock()
    conn.transaction = MagicMock(return_value=make_transaction_cm())
    return conn
...
async def test_get_user_returns_user(repository: Repository, mock_pool: AsyncMock, sample_user: User) -> None:
    """Get user joins the normalized topic/source tables and converts the
    aggregated row into a User model."""
    repository.pool = mock_pool
    ...
    assert "LEFT JOIN user_preferred_topics t" in sql
    assert "LEFT JOIN user_excluded_sources s" in sql
    assert "WHERE u.user_id = $1" in sql
...
@pytest.mark.asyncio
async def test_save_user_writes_inside_a_transaction(
    repository: Repository, mock_pool: AsyncMock, mock_connection: AsyncMock, sample_user: User
) -> None:
    """Save user acquires one connection and opens exactly one transaction
    covering the users / user_preferred_topics / user_excluded_sources writes
    -- the atomicity guarantee that replaces the old single-statement JSONB
    upsert now that preferences live in normalized join tables."""
    mock_pool.acquire = MagicMock(return_value=make_acquire_cm(mock_connection))
    repository.pool = mock_pool
...
@pytest.mark.asyncio
async def test_save_user_upserts_base_row_and_replaces_topics_and_sources(
    repository: Repository, mock_pool: AsyncMock, mock_connection: AsyncMock, sample_user: User
) -> None:
    """Save user upserts the base row, then fully replaces (delete + insert)
    both join tables so a saved preference set never merges with a stale one."""
    mock_pool.acquire = MagicMock(return_value=make_acquire_cm(mock_connection))
    repository.pool = mock_pool

    await repository.save_user(sample_user)

    execute_calls = [c.args for c in mock_connection.execute.call_args_list]
    assert any(
        "INSERT INTO users" in call[0]
        and "ON CONFLICT (user_id) DO UPDATE" in call[0]
        and call[1:] == (sample_user.user_id, sample_user.preferred_length)
        for call in execute_calls
    )
    assert any(
        "DELETE FROM user_preferred_topics" in call[0] and call[1] == sample_user.user_id
        for call in execute_calls
    )
    assert any(
        "DELETE FROM user_excluded_sources" in call[0] and call[1] == sample_user.user_id
        for call in execute_calls
    )

    mock_connection.executemany.assert_any_await(
        """INSERT INTO user_preferred_topics (user_id, topic) VALUES ($1, $2)""",
        [(sample_user.user_id, topic.value) for topic in sample_user.preferred_topics],
    )
    mock_connection.executemany.assert_any_await(
        """INSERT INTO user_excluded_sources (user_id, source) VALUES ($1, $2)""",
        [(sample_user.user_id, source) for source in sample_user.excluded_sources],
    )

@pytest.mark.asyncio
async def test_save_user_skips_empty_topic_or_source_inserts(
    repository: Repository, mock_pool: AsyncMock, mock_connection: AsyncMock
) -> None:
    """Save user does not call executemany with an empty list -- a user with
    no preferences yet should only get the DELETE (clearing any old rows)."""
    mock_pool.acquire = MagicMock(return_value=make_acquire_cm(mock_connection))
    repository.pool = mock_pool
    user = User(user_id="new-user", preferred_topics=[], excluded_sources=[])

    await repository.save_user(user)

    mock_connection.executemany.assert_not_awaited()

@pytest.mark.asyncio
async def test_save_user_raises_runtime_error_on_database_failure(
    repository: Repository, mock_pool: AsyncMock, sample_user: User
) -> None:
    """Save user converts PostgreSQL errors (here, failing to acquire a
    connection) to RuntimeError."""
    mock_pool.acquire = MagicMock(side_effect=asyncpg.PostgresError("database error"))
    repository.pool = mock_pool
    with pytest.raises(RuntimeError, match="Failed to save user"):
        await repository.save_user(sample_user)

@pytest.mark.asyncio
async def test_save_user_rolls_back_on_mid_transaction_failure(
    repository: Repository, mock_pool: AsyncMock, mock_connection: AsyncMock, sample_user: User
) -> None:
    """If a statement inside the transaction fails (e.g. the topics insert),
    the error still surfaces as a RuntimeError -- the transaction context
    manager itself is responsible for the rollback."""
    mock_pool.acquire = MagicMock(return_value=make_acquire_cm(mock_connection))
    repository.pool = mock_pool
    mock_connection.executemany.side_effect = asyncpg.PostgresError("constraint violation")
